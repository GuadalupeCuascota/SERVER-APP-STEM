...
describe('Fabric Essentials [fabric/fabric.spec]', () => {
    ...
    it('Check connect works', (done) => {
        bus.fabric.connect((sessionId) => {
    ...
    it('Check connect works with different values', (done) => {
        bus.fabric.connect((sessionId) => {
            expect(sessionId).not.toBeNull();
            done();
        }, () => {
        }, 'somehost', 8080, '/somewhere', false, '/jabber', '/blabber', 1, true);
    });
    it('Check disconnect works', (done) => {
        bus.fabric.connect(() => {
    ...
    it('Check connect works when called twice', (done) => {
        let counter = 0;
        bus.fabric.connect(() => {
            counter++;
            if (counter > 1) {
    ...
    it('Check offline event listeners work', (done) => {
        let connectCount = 0;
        bus.fabric.connect(() => {
            connectCount++;
    ...
    xit('Check online event listeners work', (done) => {
        let connectCount = 0;
    ...
    it('Check for connection state change to connected', (done) => {
        bus.fabric.whenConnectionStateChanges('testhost:12345/fabric')
            .subscribe((state) => {
    ...
    it('Check fabric version API works.', (done) => {
        ...
        bus.fabric.connect(() => {
            bus.fabric.getFabricVersion('testhost:12345/fabric').subscribe((id) => {
    ...
    it('Check we cannot get a version from the fabric, if we are not connected', (done) => {
        bus.fabric.getFabricVersion('testhost:12345/fabric').subscribe((value) => {
    ...
    it('Check if XSRF token can be read from store', (done) => {
        const id = GeneralUtil.genUUID();
        const store = bus.stores.createStore(Stores.XsrfToken);
        // fetch the token from store
        store.whenReady(() => {
            expect(bus.fabric.getXsrfToken()).toBe(id);
            done();
        });
        store.populate(new Map([[bus.fabric.getXsrfTokenStoreKey(), id]]));
    });
    ...
    it('Check that when XSRF token is null, an empty value is returned from getXsrfToken()', (done) => {
        const store = bus.stores.createStore(Stores.XsrfToken);
        store.whenReady(() => {
