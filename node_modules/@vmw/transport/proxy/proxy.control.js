 ...
 */
import { ProxyType, BusProxyMessage, ProxyControlType } from './message.proxy.api';
...
export class ProxyControlImpl {
    ...
    constructor(bus, config) {
        ...
        this.bus = bus;
        this.config = config;
        this.proxyControlChannel = '__proxycontrol__';
        this.proxyId = EventBus.id;
        this.devMode = false;
        /**
         ...
         * @type {boolean}
         */
        this.targetAllFramesValue = true;
        ...
        if (config) {
            ...
            this.bus.logger.error('Message Proxy cannot start. No configuration has been set.', this.getName());
            return;
        }
        // connect to monitor;
        this.monitorChannel = this.bus.api.getChannel(MonitorChannel.stream);
        // create bus instance map
        this.knownBusInstances = new Map();
    ...
    informParentChildBusIsListening() {
        const proxyCommand = {
    ...
    informParentChildBusStoppedListening() {
        const proxyCommand = {
    ...
    sendMessageToParent(message, chan) {
        this.bus.logger.debug(`Authorized message received on: [${chan}], sending to parent frame`, this.getName());
        const proxyMessage = new BusProxyMessage(JSON.stringify(message), chan, message.type, EventBus.id);
        parent.window.postMessage(proxyMessage, '*');
    }
    sendControlToParent(control) {
        this.bus.logger.debug(`Authorized control message command: '${control.command}' ` +
            `received, sending to parent frame`, this.getName());
        const proxyMessage = new BusProxyMessage(JSON.stringify(control), this.proxyControlChannel, MessageType.MessageTypeControl, EventBus.id);
        proxyMessage.control = control.command;
        parent.window.postMessage(proxyMessage, '*');
    }
    sendMessageToChildFrames(message, chan) {
        this.bus.logger.debug(`Authorized message received on: [${chan}], from ${message.sender}, sending to children`, this.getName());
        const proxyMessage = new BusProxyMessage(JSON.stringify(message), chan, message.type, message.sender);
    ...
    parentEventHandler(event) {
        ...
        if (!originOk) {
            this.bus.logger.warn(`Event bus broadcast refused by bus ${EventBus.id}, origin not registered: ${event.origin}`, this.getName());
            return;
        }
        ...
        if (event.data && event.data !== '') {
            ...
            if (data.hasOwnProperty('channel') && data.hasOwnProperty('type') && data.hasOwnProperty('payload')) {
                ...
                if (data.channel === null || data.channel === '') {
                    this.bus.logger.warn('Proxy Message invalid - ignored. No channel supplied', this.getName());
                    return;
                }
                if (data.type === null || data.type === '') {
                    this.bus.logger.warn('Proxy Message invalid - ignored. No message type supplied', this.getName());
                    return;
                }
                if (data.payload === null || data.payload === '') {
                    this.bus.logger.warn('Proxy Message invalid - ignored. Payload is empty', this.getName());
                    return;
                }
                // looks like the message is valid, lets check the channel for authorization.
                if (!this.validateChannel(data.channel)) {
                    this.bus.logger.warn('Proxy Message valid, but channel is not authorized: [' + data.channel + ']', this.getName());
                    return;
                }
                else {
                        ...
                        switch (data.control) {
                            ...
                            case ProxyControlType.RegisterEventBus:
                                ...
                                if (!this.knownBusInstances.has(payload.body)) {
                                    ...
                                    this.bus.logger.info(`Child Event Bus Registered: ${payload.body}`, this.getName());
                                    // inform monitor
                                    mo = new MonitorObject().build(MonitorType.MonitorChildProxyRegistered, this.proxyControlChannel, payload.body, payload.body);
                            ...
                            case ProxyControlType.BusStartListening:
                                ...
                                // inform monitor
                                mo = new MonitorObject().build(MonitorType.MonitorChildProxyListening, this.proxyControlChannel, payload.body, payload.body);
                                this.bus.api.getMonitorStream().send(new Message().response(mo));
                                break;
                            ...
                            case ProxyControlType.BusStopListening:
                                ...
                                // inform monitor
                                mo = new MonitorObject().build(MonitorType.MonitorChildProxyNotListening, this.proxyControlChannel, payload.body, payload.body);
                                this.bus.api.getMonitorStream().send(new Message().response(mo));
                                break;
                            default:
                                break;
                ...
                this.bus.logger.debug('Message Ignored, not intended for the bus.', this.getName());
            ...
            this.bus.logger.debug('Message Ignored, it contains no payload', this.getName());
            return;
        }
    }
    validateChannel(requestedChannel) {
...
//# sourceMappingURL=proxy.control.js.map