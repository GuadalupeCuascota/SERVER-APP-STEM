...
import { BusProxyMessage, ProxyType } from './message.proxy.api';
...
describe('Proxy Controls [proxy/proxy.control.ts]', () => {
    ...
    describe('Validation & Rule Checking', () => {
        ...
        it('Proxy ignores messages with unregistered origin.', (done) => {
            ...
            bus.enableMessageProxy({
                protectedChannels: ['auth-chan1'],
                proxyType: ProxyType.Parent,
                acceptedOrigins: ['http://something.else'],
                targetAllFrames: true,
                parentOrigin: null,
                targetSpecificFrames: null
            });
            ...
            bus.api.tickEventLoop(() => {
                expect(log.warn)
                    .toHaveBeenCalledWith(`Event bus broadcast refused by bus ${EventBus.id}, origin not registered: http://localhost:9876`, EventBus.id);
                done();
            }, 30);
        });
        it('Check child bus can be activated and deactivated correctly', (done) => {
            ...
            control = bus.enableMessageProxy({
                ...
                proxyType: ProxyType.Child,
                acceptedOrigins: ['http://localhost:9876'],
                targetAllFrames: true,
        ...
        it('Proxy accepts a valid message, but drops without a channel defined.', (done) => {
            ...
            bus.api.tickEventLoop(() => {
                const invalidProxyMessage = new BusProxyMessage('time for a walk, pups', '', MessageType.MessageTypeRequest);
        ...
        it('Proxy accepts a valid message, but drops without a type defined.', (done) => {
            ...
            bus.api.tickEventLoop(() => {
                const invalidProxyMessage = new BusProxyMessage('who wants a treat?', 'somechan', null);
                window.postMessage(invalidProxyMessage, '*'); // send message, origin is local karma
            });
            bus.api.tickEventLoop(() => {
                expect(bus.logger.warn)
        ...
        it('Proxy accepts a valid message, but drops with an empty payload.', (done) => {
            ...
            bus.api.tickEventLoop(() => {
                const invalidProxyMessage = new BusProxyMessage('', 'somechan', MessageType.MessageTypeRequest);
                window.postMessage(invalidProxyMessage, '*'); // send message, origin is local karma
            });
            bus.api.tickEventLoop(() => {
                expect(bus.logger.warn)
        ...
        it('Proxy accepts a valid message, but drops without a payload defined', (done) => {
            ...
            bus.api.tickEventLoop(() => {
                const invalidProxyMessage = new BusProxyMessage(null, 'somechan', MessageType.MessageTypeRequest);
                window.postMessage(invalidProxyMessage, '*'); // send message, origin is local karma
            });
            bus.api.tickEventLoop(() => {
                expect(bus.logger.warn)
        ...
        it('Proxy accepts a valid message, but drops if the channel is not authorized', (done) => {
            ...
            bus.api.tickEventLoop(() => {
                const invalidProxyMessage = new BusProxyMessage('mr. hacker', 'somechan', MessageType.MessageTypeRequest);
                window.postMessage(invalidProxyMessage, '*'); // send message, origin is local karma
            });
            bus.api.tickEventLoop(() => {
                expect(bus.logger.warn)
    ...
    describe('Parent Proxying Behaviors', () => {
        ...
        it('If a valid command message is received for an authorized channel, it is proxied.', (done) => {
            ...
            bus.api.tickEventLoop(() => {
                const validProxyMessage = new BusProxyMessage(JSON.stringify(new Message().request('is it dinner time?')), 'ember-radio', MessageType.MessageTypeRequest);
        ...
        it('If a valid response message is received for an authorized channel, it is proxied.', (done) => {
            ...
            bus.api.tickEventLoop(() => {
                const validProxyMessage = new BusProxyMessage(JSON.stringify(new Message().request('can I get some more milk?')), 'melody-radio', MessageType.MessageTypeResponse);
                window.postMessage(validProxyMessage, '*'); // send message, origin is local karma
            });
        });
        it('If a valid error message is received for an authorized channel, it is proxied.', (done) => {
            ...
            bus.api.tickEventLoop(() => {
                const validProxyMessage = new BusProxyMessage(JSON.stringify(new Message().request('I need to be changed')), 'melody-cry', MessageType.MessageTypeError);
                window.postMessage(validProxyMessage, '*'); // send message, origin is local karma
            });
        });
        it('If operating in parent mode and all frames are targeted, proxy should proxy.', (done) => {
            ...
            const frameHandler = (evt) => {
                const proxyMessage = evt.data;
                const message = JSON.parse(proxyMessage.payload);
...
//# sourceMappingURL=proxy.control.spec.js.map