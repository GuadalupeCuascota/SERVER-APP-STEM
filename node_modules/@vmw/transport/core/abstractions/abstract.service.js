 ...
 */
import { ORG_ID, ORGS } from '../../bus.api';
 ...
 */
export class CallerArgs {
    constructor(uuid, from = '🔋SYNTHETIC🔋', version = 1) {
        this.uuid = uuid;
        this.from = from;
 ...
 */
export class AbstractService extends AbstractBase {
    /**
     * super()
     *
     ...
     */
    constructor(name, serviceChannel, broadcastChannel) {
        super(name);
        ...
        this.requestConverterMap = new Map(HTTP_REQUEST_MAP);
    ...
    listenToRequestStream() {
        this.log.info(`🌎 Service Adaptor: ${this.name} (${this.id}) online and listening on '${this.serviceChannel}'`, this.getName());
        this.requestStream = this.bus.listenRequestStream(this.serviceChannel, this.getName());
        this.requestStreamSub = this.requestStream.handle((requestObject, args) => {
     ...
     */
    postResponse(channel, responseObject, args) {
        if (args) {
     ...
     */
    buildAPIRequest(requestCommand, payload, uuid = GeneralUtil.genUUID(), version = 1) {
     ...
     */
    initializeServiceCallHandling() {
        this.serviceCall = (channel, requestObject, successHandler, failureHandler, callerArgs) => {
            const messageHandler = this.bus
                .requestOnceWithId(GeneralUtil.genUUID(), channel, requestObject, null, this.getName());
            messageHandler
                .handle((callResponseObject, args) => {
    ...
    initializeApiHandling() {
        ...
        //
        this.apiBridge = (apiObject, httpOp, uri, body, successHandler, failureHandler, apiClass, apiArgs) => {
            if (!this.requestConverterMap.has(httpOp)) {
                this.log.error('FATAL: Invalid RestRequest provided to AbstractService.apiBridge(): ' + httpOp, this.getName());
                return;
            }
            if (!apiArgs) {
            ...
            const restRequestObject = new RestObject(this.requestConverterMap.get(httpOp), uri, // fully formed URI with path params and query params
            body, apiObject.getHeaders(), // headers
            {}, // queryStringParams
            {}, // pathParams
            apiClass, this.getName());
            this.serviceCall(RestService.channel, restRequestObject, (restResponseObject) => {
     ...
     */
    get callerOrgId() {
        const store = this.storeManager.getStore(ORGS);
        let orgId = 'orgId not set!';
        if (store) {
     ...
     */
    genApiObject(requestObject, responseObject) {
        return new ApiObject(requestObject, responseObject);
    }
    /**
     ...
     * @param payload
     */
    broadcastResponse(channel, payload) {
     ...
     * @param notification
     */
    broadcastNotification(channel, notification) {
