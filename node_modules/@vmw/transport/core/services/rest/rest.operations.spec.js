...
import { HttpRequest, RestError } from './rest.model';
import { GeneralUtil } from '../../../util/util';
import { BusTestUtil } from '../../../util/test.util';
import { BrokerConnector, BrokerConnectorChannel, StompClient, StompConfig } from '../../../bridge';
describe('Transport Rest Operations [cores/services/rest/rest.operations]', () => {
    let bus;
    let operations;
    const connectToMockBroker = () => {
        const configuration = new StompConfig('/somewhere', 'somehost', 12345, '', '', false, null);
    ...
    it('Check global headers can be sent by operation.', (done) => {
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            const restObject = msg.payload;
    ...
    it('Check cors and creds can be disabled (not for prod use)', (done) => {
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            const restObject = msg.payload;
    ...
    it('Check host and scheme can be set / changed', (done) => {
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            const restObject = msg.payload;
    ...
    it('HTTP Request can be made', (done) => {
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            const restObject = msg.payload;
    ...
    it('HTTP Request can be made and mocked response returned', (done) => {
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            const restObject = msg.payload;
            expect(restObject.request).toEqual(HttpRequest.Get);
            expect(restObject.uri).toEqual('http://melody.rose');
    ...
    it('Check request ID is able to be set correctly', (done) => {
    ...
    it('Check request is wrapped if the channel is galactic', (done) => {
        const id = GeneralUtil.genUUID();
        let bcSub;
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            const apiRequest = msg.payload;
            const restObject = apiRequest.payload;
    ...
    it('Check request is wrapped if the channel is galactic - and apiClass supplied', (done) => {
        const id = GeneralUtil.genUUID();
        let bcSub;
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            const apiRequest = msg.payload;
    ...
    it('Ensure that wire based fabric requests from backend RestService instances are unpacked correctly.', (done) => {
        const id = GeneralUtil.genUUID();
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            if (msg.isRequest()) {
                const restObject = msg.payload;
                ...
                if (msg.isRequest()) {
                    ...
                    bus.sendResponseMessageWithId(RestService.channel, bus.fabric.generateFabricResponse(restObject.id, restObject.response), msg.id);
                }
            }
        });
        operations.restServiceRequest({
    ...
    it('Ensure that wire based fabric errors from backend RestService instances are unpacked correctly.', (done) => {
        const id = GeneralUtil.genUUID();
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            if (msg.isRequest()) {
                const restObject = msg.payload;
                expect(restObject.request).toEqual(HttpRequest.Get);
                expect(restObject.uri).toEqual('http://melody.rose');
                const err = new RestError('oh dear, the computer said no.');
                // fake response
                if (msg.isRequest()) {
                    bus.sendErrorMessageWithId(RestService.channel, bus.fabric.generateFabricResponse(restObject.id, err, true, 500, 'error'), msg.id);
                }
            }
        });
        operations.restServiceRequest({
    ...
    it('Ensure that local errors are handled correctly.', (done) => {
        const id = GeneralUtil.genUUID();
        let sub = bus.api.getChannel(RestService.channel).subscribe((msg) => {
            if (msg.isRequest()) {
                const restObject = msg.payload;
                expect(restObject.request).toEqual(HttpRequest.Get);
                expect(restObject.uri).toEqual('http://melody.rose');
                const err = new RestError('oh dear, the computer said no... again');
                // fake response
                if (msg.isRequest()) {
                    bus.sendErrorMessageWithId(RestService.channel, err, msg.id);
                }
            }
        });
        operations.restServiceRequest({
...
//# sourceMappingURL=rest.operations.spec.js.map