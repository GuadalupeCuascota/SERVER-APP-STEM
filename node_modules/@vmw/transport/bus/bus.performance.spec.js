...
xdescribe('Transport Performance Testing [bus/bus.performance.spec.ts]', () => {
    ...
    it('Validate 100 channels can be created, subscribed to, un-subscribed from and destroyed in less than 10ms', () => {
        ...
        for (let x = 0; x < 100; x++) {
            const channelName = GeneralUtil.genUUID();
            const channel = bus.api.getChannel(channelName);
...
function printTimeBefore(time) {
    if (printTimeLogs) {
        bus.logger.info('time before: ' + time);
    }
}
function printTimeStep(step, time) {
    if (printTimeLogs) {
        bus.logger.info('time ' + step + ': ' + time);
    }
}
function printTimeAfter(time) {
    if (printTimeLogs) {
        bus.logger.info('time after: ' + time);
    }
}
function printTotalTime(time) {
    if (printTimeLogs) {
        bus.logger.info('execution time total ms: ' + time);
    }
}
function runApiPerformanceTestOverSocket(host, loops, done, expectedRunTime, command) {
    bus = BusTestUtil.bootBusWithOptions(LogLevel.Info, true);
    bus.logger.setStylingVisble(false);
    ...
    bus.connectBridge(() => {
        theBridgeIsReady();
    }, '/transport', '/topic', '/queue', 1, host, 8080, '/pub');
}
function runCustomApiPerformanceTestOverSocket(host, loops, done, expectedRunTime) {
    runApiPerformanceTestOverSocket(host, loops, done, expectedRunTime, 'CustomLogic');
}
function runApiPerformanceTestOverXHR(host, loops, done, expectedRunTime, command) {
    bus = BusTestUtil.bootBusWithOptions(LogLevel.Info, true);
    bus.logger.setStylingVisble(false);
    const chan = GeneralUtil.genUUID();
    let responseCount = 0;
    let timeBefore, timeAfter;
    bus.listenRequestStream(chan).handle((request, args) => {
...
function runCustomApiPerformanceTestOverXHR(host, loops, done, expectedRunTime) {
    runApiPerformanceTestOverXHR(host, loops, done, expectedRunTime, 'seed/custom');
}
//# sourceMappingURL=bus.performance.spec.js.map