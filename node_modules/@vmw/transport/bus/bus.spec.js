 ...
 */
import { TransportEventBus, BrokerConnectorChannel } from '../';
import { LogLevel } from '../log/logger.model';
import { Message } from './model/message.model';
import { StompParser } from '../bridge/stomp.parser';
import { StompClient } from '../bridge/stomp.client';
import { MonitorType } from './model/monitor.model';
...
describe('TransportEventBus [bus/bus.ts]', () => {
    ...
    it('Should return same Channel for a new subscriber', () => {
        const channel = bus.api.getChannelObject(testChannel);
        const channel2 = bus.api.getChannelObject(testChannel);
        expect(channel)
            .not
            .toBeUndefined();
        expect(channel2)
    ...
    it('Should send and receive data over the message bus', (done) => {
        const channel = bus.api.getRequestChannel(testChannel);
    ...
    it('Should send and receive error over the message bus', (done) => {
        const channel = bus.api.getErrorChannel(testChannel);
        channel.subscribe((message) => {
    ...
    it('Should send and receive an error over the message bus', (done) => {
        const channel = bus.api.getChannel(testChannel);
        channel.subscribe(null, (error) => {
    ...
    it('Should send data over the message bus to 2 subscribers (one-to-many)', (done) => {
        ...
        const channel = bus.api.getResponseChannel(testChannel);
        channel.subscribe((message) => {
            expect(message.payload).toBe(testData);
            doneCaller();
        });
        const channel2 = bus.api.getChannel(testChannel);
    ...
    it('Check countListeners() is accurate against high level API (single response)', (done) => {
        ...
        */
        ...
        const handlerOnce = bus.listenOnce('puppers');
        expect(bus.api.countListeners()).toEqual(4);
        handlerOnce.handle((msg) => {
    ...
    it('Check countListeners() is accurate against high level API (stream response)', (done) => {
        ...
        */
        expect(bus.api.countListeners()).toEqual(3);
        const handler = bus.listenStream('puppers');
        expect(bus.api.countListeners()).toEqual(4);
        let count = 0;
        handler.handle((msg) => {
        ...
        bus.sendResponseMessage('puppers', 'chicken');
        bus.sendResponseMessage('puppers', 'chicken');
        bus.api.tickEventLoop(() => bus.closeChannel('puppers'), 5);
        // should have settled
        bus.api.tickEventLoop(() => {
            expect(bus.api.countListeners()).toEqual(3);
    ...
    it('Check createResponder() handles errors with error handler', (done) => {
        expect(bus.api.countListeners()).toEqual(3);
        const responder = bus.respondOnce('puppers');
        responder.generate(null, (msg) => {
    ...
    it('Check createResponder() handles tick() corectly', (done) => {
        let count = 0;
        const responder = bus.respondStream('puppers');
        responder.generate((msg) => {
    ...
    it('Check createMessageHandler() can handle observable errors', (done) => {
        ...
        const chan = bus.api.getChannelObject('puppers');
    ...
    it('Check createMessageHandler() does not tick() on a closed stream', (done) => {
        ...
        const handler = bus.listenOnce('puppers');
        handler.handle((msg) => {
    ...
    it('Check createMessageHandler() observable handles default (empty) message type', (done) => {
        const handler = bus.listenOnce('puppers');
        handler.getObservable().subscribe((val) => {
    ...
    describe('Simple API Tests', () => {
        it('respondOnce() and requestOnce()', (done) => {
        ...
        it('respondOnce() and requestOnce() on differing command/response channels.', (done) => {
        ...
        it('command once with a different return channel', (done) => {
        ...
        it('command stream with a different return channel', (done) => {
            const channel1 = '#test-channel1';
            const channel2 = '#test-channel2';
            const handler1 = bus.listenRequestStream(channel1);
            const sub1 = handler1.handle((request) => {
                expect(request).toEqual('strawbarita');
                bus.api.sendResponse(channel2, 'margarita');
                sub1.unsubscribe();
            });
            const handler2 = bus.requestStream(channel1, 'strawbarita', channel2);
            const sub2 = handler2.handle((resp) => {
                expect(resp).toEqual('margarita');
                sub2.unsubscribe();
                done();
            });
        });
        it('respondStream() and requestStream()', (done) => {
            let responder = bus.respondStream(testChannel);
            responder.generate((request) => {
                return ++request;
            });
            let requester = bus.requestStream(testChannel, 1);
            requester.handle((resp) => {
                if (resp < 10) {
        ...
        it('listenOnce() [ listen for a single response ]', (done) => {
            let c = 0;
            let h = 0;
        ...
        it('listenStream() [ listen for multiple response messages on channel ]', (done) => {
            let h = 0;
        ...
        it('listenRequestOnce() [listen to a single command]', (done) => {
            let c = 0;
            let h = 0;
        ...
        it('listenRequestStream() [ listen for multiple command messages on a channel ]', (done) => {
            let h = 0;
        ...
        it('sendRequest() [ simple API wrapper for sending command messages (wraps MessageHandlerConfig) ]', (done) => {
        ...
        it('sendResponse() [ simple API wrapper for sending response messages (wraps MessageHandlerConfig) ]', (done) => {
        ...
        it('sendRequestMessage() [ lower level wrapper for sending command messages ]', (done) => {
            let _chan = bus.api.getRequestChannel(testChannel, 'sendRequestMessage()');
            let p = _chan.subscribe((m) => {
        ...
        it('sendRequestMessageWithId() [ test we can send a command with an ID and pick it up] ', (done) => {
        ...
        it('sendRequestMessageWithIdAndVersion() [ test we can send a command with an' +
            ' ID + version and pick it up] ', (done) => {
            const id = GeneralUtil.genUUIDShort();
            const chan = bus.api.getChannel(testChannel);
            chan.subscribe((msg) => {
                expect(msg.id).toEqual(id);
        ...
        it('sendResponseMessage() [ lower level wrapper for sending response messages ]', (done) => {
        ...
        it('sendRequestMessageWithId() [ test we can send a command with and ID and pick it up] ', (done) => {
            const id = GeneralUtil.genUUIDShort();
            const chan = bus.api.getChannel(testChannel);
            chan.subscribe((msg) => {
                expect(msg.id).toEqual(id);
        ...
        it('sendErrorMessage() [ lower level wrapper for sending error messages ]', (done) => {
        ...
        it('Should be able to mix old and new APIs together [sendRequest() and subscribe()]', (done) => {
        ...
        it('Should be able to mix old and new APIs together [send() and listenOnce()]', (done) => {
        ...
        it('Should be able to mix old and new APIs together [requestOnce(), subscribe(), sendResponse()]', (done) => {
            let chan = bus.api.getRequestChannel(testChannel, 'mixOldAndNew');
            chan.subscribe((msg) => {
        ...
        it('Should be able to mix old and new APIs together [requestStream(), subscribe(), sendResponse()]', (done) => {
            let chan = bus.api.getRequestChannel(testChannel, 'mixOldAndNew');
            chan.subscribe((msg) => {
        ...
        it('Should be able to mix old and new APIs together [respondOnce(), send(), subscribe()]', (done) => {
        ...
        it('Should be able to mix old and new APIs together [respondStream(), requestStream(), tick(), subscribe()]', (done) => {
            let tickCount = 0;
            let responder = bus.respondStream(testChannel);
            responder.generate((request) => {
                return ++request;
            ...
            let chan = bus.api.getResponseChannel(testChannel, 'mixOldAndNew');
            let sub = chan.subscribe((msg) => {
                expect(msg.payload).toBeGreaterThan(0);
                tickCount++;
            });
            let requester = bus.requestStream(testChannel, 1);
            requester.handle((val) => {
                if (val <= 9) {
                    requester.tick(val);
                    return;
                }
        ...
        it('Should be able to handle an Error [listenOnce(), sendErrorMessage()]', (done) => {
            bus.listenOnce(testChannel)
                .handle(() => {
        ...
        it('Should be able to get observable from message handler for responses', (done) => {
            bus.respondOnce(testChannel)
                .generate((val) => {
        ...
        it('Should be able to get observable from message handler for requests', (done) => {
            const handler = bus.listenOnce(testChannel);
        ...
        it('Should be able to get observable from message handler for errors', (done) => {
            const handler = bus.listenOnce(testChannel);
        ...
        it('Should be able to get observable from message handler for full channel', (done) => {
            const handler = bus.listenOnce(testChannel);
        ...
        it('Should be able to listen to multiple streams', (done) => {
            let val = 0;
        ...
        it('Should be able to get observable from message responder for requests', (done) => {
            const responder = bus.respondOnce(testChannel);
        ...
        it('Should be able to get observable from message responder for errors', (done) => {
            const responder = bus.respondOnce(testChannel);
            const obs = responder.getObservable();
            obs.subscribe(() => {
                expect(true).toBeFalsy();
        ...
        it('Responder streams should not be able to respond after closing the stream', (done) => {
            const responder = bus.respondStream('puppy-talk');
            const handler = bus.listenStream('puppy-talk');
            let counter = 0;
            responder.generate((req) => {
        ...
        it('Nothing should happen when trying to close a single responder.', (done) => {
            const responder = bus.respondOnce('puppy-talk');
            const handler = bus.listenStream('puppy-talk');
            let counter = 0;
            responder.generate((req) => {
        ...
        it('Responder should do nothing if there is an error thrown.', (done) => {
            ...
            bus.respondOnce('puppy-dinner-talk')
                .generate(() => 'tonights dinner is steak');
            bus.api.error('puppy-dinner-talk', 'oh no! the steak is all gone!');
            bus.api.tickEventLoop(() => {
                expect(bus.api.loggerInstance.error)
                    .toHaveBeenCalledWith('responder caught error, discarding.', 'EventBus');
                done();
            }, 10);
        });
        it('createMessageHandler() should log error if valid message comes in without success handler defined', (done) => {
            spyOn(bus.api.loggerInstance, 'error').and.callThrough();
            bus.api.enableMonitorDump(true);
            bus.api.silenceLog(false);
            bus.api.setLogLevel(LogLevel.Error);
            ...
            bus.api.tickEventLoop(() => {
                expect(bus.api.loggerInstance.error)
                    .toHaveBeenCalledWith('unable to handle response, no handler function supplied', 'EventBus');
                done();
            }, 50);
        });
        it('createMessageHandler() should log error if an error comes in without error  handler defined', (done) => {
            spyOn(bus.api.loggerInstance, 'error').and.callThrough();
            bus.api.enableMonitorDump(true);
            bus.api.silenceLog(false);
            bus.api.setLogLevel(LogLevel.Error);
            ...
            bus.api.tickEventLoop(() => {
                expect(bus.api.loggerInstance.error)
                    .toHaveBeenCalledWith('unable to handle error, no error handler function supplied', 'EventBus');
                done();
            }, 50);
        });
        /* Simple API's with ID's and Versions */
        it('requestOnceWithId() should work as expected.', (done) => {
            let idCount = 0;
            let allCount = 0;
        ...
        it('requestStreamWithId() should work as expected.', (done) => {
            let idCount = 0;
            let allCount = 0;
        ...
        it('check ID and sender is correctly passed through with handler via MessageArgs.', (done) => {
            const chan = 'pup-talk';
            bus.respondStream(chan, chan, 'ember')
                .generate((payload, args) => {
                expect(args.from).toEqual('cotton');
                expect(payload).toEqual('whats the chat?');
                return 'bark';
            });
            bus.listenStream(chan).handle((response, args) => {
        ...
        it('check ID, version and sender is correctly passed through with handler via MessageArgs.', (done) => {
            const chan = 'pup-talk';
            bus.respondStream(chan, chan, 'ember')
                .generate((payload, args) => {
                expect(args.from).toEqual('cotton');
            ...
            bus.sendRequestMessageWithIdAndVersion('pup-talk', 'whats the chat?', '1234', 99, 'cotton');
        });
    });
    /**
    ...
    describe('Broker Connector & Galactic Tests', () => {
        it('connectBroker() reacts correctly to connected messages', (done) => {
            const readyHandler = (sessionId) => {
                expect(sessionId).toEqual('squeaky-chew');
                done();
            };
            bus.connectBridge(readyHandler, '/somewhere', '/topic', '/queue', 0);
            // fake the broker sending back a valid connection and session.
            bus.sendResponseMessage(BrokerConnectorChannel.connection, StompParser.generateStompBusCommand(StompClient.STOMP_CONNECTED, 'squeaky-chew'));
        });
        it('connectBroker() logs unexpected stomp command', (done) => {
            const readyHandler = () => {
                done();
            };
            spyOn(bus.api.loggerInstance, 'info').and.callThrough();
            ...
            bus.sendResponseMessage(BrokerConnectorChannel.connection, StompParser.generateStompBusCommand(StompClient.STOMP_ABORT, 'squeaky-chew'));
            bus.api.tickEventLoop(() => {
                expect(bus.api.loggerInstance.info)
                    .toHaveBeenCalledWith('connection handler received command message: ABORT', 'EventBus');
                readyHandler();
            }, 50);
        });
        describe('when calling connectBridge() with advancedConfig param', () => {
            it('populates the correct stompConfig parameters', () => {
                ...
                let advancedConfig = {
                    ...
                    startIntervalFunction: (handler, timeout, ...args) => { return 67; }
                };
                ...
                bus.connectBridge(() => { }, '/somewhere', '/topic', '/queue', 0, undefined, undefined, undefined, undefined, undefined, undefined, undefined, advancedConfig);
                expect(bus.requestStreamWithId).toHaveBeenCalled();
                let stompCmd = requestStreamSpy.calls.mostRecent().args[2];
                const stompConfig = stompCmd.payload;
        ...
        it('listenGalacticStream() works correctly.', (done) => {
            const monitor = bus.api.getMonitor()
                .subscribe((message) => {
                const mo = message.payload;
                switch (mo.type) {
        ...
        it('listenGalacticStream() works correctly with multiple subscribers.', (done) => {
        ...
        it('sendGalacticMessage() works correctly.', (done) => {
            const monitor = bus.api.getMonitor()
                .subscribe((message) => {
                const mo = message.payload;
                switch (mo.type) {
        ...
        describe('markChannelAsGalactic()', () => {
            ...
            it('sets the channel to be galactic', () => {
                const channel = bus.api.getChannelObject(channelName);
