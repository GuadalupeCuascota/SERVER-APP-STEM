...
(function (Mutate) {
    ...
    Mutate["AddStuff"] = "AddStuff";
    Mutate["RemoveStuff"] = "RemoveStuff";
})(Mutate || (Mutate = {}));
describe('BusStore [store/store.model]', () => {
    ...
    describe('Galactic stores', () => {
        ...
        it('when created request store content', (done) => {
            ...
            const galacticStore = new StoreImpl(bus, 'galacticStore', 'store-sync-channel');
        ...
        it('galactic updates trigger store change handlers', (done) => {
            ...
            galacticStore.onAllChanges().subscribe((item, args) => {
        ...
        it('galactic remove updates trigger store change handlers', (done) => {
            ...
            galacticStore.onChange('item1').subscribe((item, args) => {
    ...
    it('Check cache has been set up correctly', () => {
        ...
        expect(bus.stores.getStore('string').populate(new Map())).toBeTruthy();
    });
    ...
    it('check remove() works correctly', (done) => {
        const cache = bus.stores.getStore('string');
        cache.onChange('123', State.Deleted)
            .subscribe((s) => {
    ...
    it('check onChange() works correctly', (done) => {
        ...
        let counter = 0;
        cache.onChange('magnum', State.Created)
            .subscribe((d, args) => {
        ...
        cache.onChange('fox', State.Updated)
            .subscribe((d) => {
        ...
        cache.onChange('cotton', State.Deleted)
            .subscribe((d) => {
        ...
        cache.put('magnum', new Dog('maggie', 12, 'get the kitty'), State.Created);
        cache.put('fox', new Dog('foxy pop', 11, 'get out of the pantry'), State.Updated);
        cache.put('cotton', new Dog('chickie', 6, 'where is the kitty'), State.Deleted);
    });
    it('onChange() works with no success handler supplied', (done) => {
        spyOn(log, 'error').and.callThrough();
        const store = bus.stores.createStore('dog');
        store.onChange('magnum', State.Created)
            .subscribe(null);
    ...
    it('onChange() works with unsubscriptions', (done) => {
        let counter = 0;
        const store = bus.stores.createStore('dog');
        const sub = store.onChange('magnum', State.Updated);
    ...
    it('check onAllChanges() works correctly', (done) => {
        const cache = bus.stores.createStore('Dog');
        let counter = 0;
        cache.onAllChanges(State.Created)
            .subscribe((d, args) => {
            counter++;
            if (counter === 1) {
        ...
        cache.onAllChanges(State.Updated)
            .subscribe(() => {
            counter++;
            if (counter === 7) {
                done();
        ...
        cache.put('something-else', 'not a dog!', State.Created);
        cache.put('magnum', new Dog('maggie', 12, 'get the kitty'), State.Created);
        cache.put('fox', new Dog('foxy pop', 11, 'get out of the pantry'), State.Created);
        cache.put('cotton', new Dog('chickie', 6, 'where is the kitty'), State.Created);
        cache.put('something-else-again', 'not a dog either!', State.Created);
        cache.put('fox', new Dog('foxy pop', 11, 'get off the couch!'), State.Updated);
        cache.put('cotton', new Dog('chickie', 6, 'want to go for a walk?'), State.Updated);
    });
    it('check onAllChanges() works correctly with multiple states', (done) => {
        const cache = bus.stores.createStore('Dog');
        let counter = 0;
        listen(cache, State.Created, State.Updated)
            .subscribe((d, args) => {
            counter++;
            if (counter === 1) {
    ...
    it('check onAllChanges() works correctly with all states', (done) => {
        ...
        cache.put('fox', new Dog('foxy pop', 11, 'get out of the pantry'), State.Created);
        cache.put('fox', new Dog('foxy pop', 11, 'get off the couch!'), State.Updated);
        cache.put('fox', new Dog('foxy pop', 11, 'get off the couch!'), State.Deleted);
    });
    it('check mutate() works with correct success handling', (done) => {
        const cache = bus.stores.createStore('Dog');
        let d = new Dog('foxy', 11, 'eat it, not bury it');
        cache.put('fox', d, State.Created);
        const mutateStream = cache.onMutationRequest(new Dog(), Mutate.Update);
        mutateStream.subscribe((dog) => {
    ...
    it('check mutate() works with all mutation types', (done) => {
        ...
        cache.put('fox', d, State.Created);
        let counter = 0;
        const mutateStream = cache.onMutationRequest(new Dog());
    ...
    it('check mutate() works with correct error handling', (done) => {
        ...
        cache.mutate(d, Mutate.Update, null, (e) => {
    ...
    it('check mutate() and notifyOnMutation() works correctly', (done) => {
        const cache = bus.stores.createStore('Dog');
        let d = new Dog('maggie', 12, 'get the kitty');
        cache.put('magnum', d, State.Created);
        cache.onMutationRequest(new Dog(), Mutate.Update)
            .subscribe((dog) => {
        ...
        cache.onChange('magnum', State.Updated)
            .subscribe((dog) => {
    ...
    it('check mutatorErrorHandler works so mutator can send errors back to subscribers.', (done) => {
        const cache = bus.stores.createStore('Dog');
        let d = new Dog('chicken', 6, 'go find the kitty');
        cache.put('cotton', d, State.Created);
        const stream = cache.onMutationRequest(new Dog(), Mutate.Update);
        stream.subscribe((dog) => {
    ...
    it('check whenReady() and initialize() work.', (done) => {
        ...
        let d = new Dog('stinky', 12, 'where is your ball');
        cache.put('magnum', d, State.Created);
        cache.initialize();
    });
...
class Dog {
    constructor(name, age, commonPhrase) {
        this.name = name;
        this.age = age;
