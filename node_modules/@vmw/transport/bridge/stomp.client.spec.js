...
import { StompConfig } from './stomp.model';
...
describe('Stomp Client [stomp.client]', () => {
    ...
    let connected;
    let subId;
    let testDestination = 'test';
    let log;
    beforeEach(() => {
        config = new StompConfig('spogglepop', 'nowhere', 12345, 'user', 'password', false, null, false);
    ...
    describe('Given we can import the client', () => {
        it('We should be able to instantiate it', (done) => {
        ...
        it('We should be able to connect', (done) => {
        ...
        it('We should be able to disconnect', (done) => {
        ...
        it('We should not be able to send a message if the socket is in the wrong state', (done) => {
        ...
        it('We should not be able to send messages if we are not connected', (done) => {
            expect(client.send(testDestination)).toBeFalsy();
            client.connect(config);
            expect(client.send(testDestination)).toBeTruthy();
            done();
        });
        it('We should not be able to open more than a single socket per client', (done) => {
    ...
    describe('Given we can connect we connect', () => {
        it('We should be able to receive connection events', (done) => {
            let connection = client.connect(config);
            connection.subscribe(() => {
                expect(true).toBeTruthy();
                done();
        ...
        it('We should be able to receive error events', (done) => {
    ...
    describe('Given we can connect we connect and receive events', () => {
        it('We should be able send array buffer as string', (done) => {
            client.connect(config)
                .pipe(mergeMap(() => {
            ...
            client.socketSubscriptionObserver.subscribe(() => {
                client.send(testDestination, { subscription: subId }, StompParser.stringToArrayBuffer('binary fun in the sun'));
            });
        });
        it('We should be able process an array buffer if triggered by the user', (done) => {
            client.connect(config)
                .pipe(mergeMap(() => {
                return client.subscribeToDestination(testDestination, subId);
            })).subscribe((msg) => {
            ...
            client.socketSubscriptionObserver.subscribe(() => {
                let binMessage = StompParser.marshal(StompClient.STOMP_MESSAGE, { subscription: subId }, StompParser.stringToArrayBuffer('what a lovely buffer'));
                client.clientSocket.triggerEvent('message', [{ data: binMessage }]);
            });
        });
        it('We should be able process an array buffer if triggered by the socket', (done) => {
            client.connect(config)
                .pipe(mergeMap(() => {
                return client.subscribeToDestination(testDestination, subId);
            })).subscribe((msg) => {
            ...
            client.socketSubscriptionObserver.subscribe(() => {
                ...
                client.clientSocket.triggerEvent('message', [
                    {
                        data: StompParser.stringToArrayBuffer(StompParser.marshal(newMessage.command, newMessage.headers, newMessage.body))
                    }
                ]);
            });
        });
        it('CONNECT message should include headers', (done) => {
        ...
        it('We should be able to subscribe successfully', (done) => {
            client.connect(config).subscribe(() => {
                client.subscribeToDestination(testDestination, subId);
            });
            client.socketSubscriptionObserver.subscribe((message) => {
                expect(message.command).toEqual(StompClient.STOMP_SUBSCRIBE);
                done();
            });
        });
        it('We should be able to unsubscribe successfully', (done) => {
            client.connect(config).subscribe(() => {
                client.subscribeToDestination(testDestination, subId)
                    .subscribe(() => {
                    expect(false).toBeTruthy();
        ...
        it('We should be able to subscribe to a destination without an additional STOMP command', (done) => {
            client.connect(config).subscribe(() => {
    ...
    describe('Given we subscribe/unsubscribe with success', () => {
        it('We should be able to send a transaction and get a receipt using a generated id', (done) => {
            client.connect(config).subscribe(() => {
                client.subscribeToDestination(testDestination, subId);
            });
            client.socketSubscriptionObserver.subscribe(() => {
                ...
                if (tx.receiptObservable) {
                    tx.receiptObservable.subscribe((message) => {
        ...
        it('We should be able to send a transaction and get a receipt using a random ID', (done) => {
            client.connect(config).subscribe(() => {
                client.subscribeToDestination(testDestination, subId);
            });
            client.socketSubscriptionObserver.subscribe(() => {
        ...
        it('We should be able to send a transaction and get a receipt and fire off an ACK', (done) => {
            config = new StompConfig('chibbleslip', 'nowhere', 12345, 'user', 'password', false, null, true);
            config.testMode = true;
            client.connect(config).subscribe(() => {
                client.subscribeToDestination(testDestination, subId);
            });
            client.socketACKObserver.subscribe((message) => {
        ...
        it('We should be able to cancel a transaction', (done) => {
            client.connect(config).subscribe(() => {
                client.subscribeToDestination(testDestination, subId);
            });
            client.socketSubscriptionObserver.subscribe(() => {
    ...
    describe('Given we can commit and abort transactions', () => {
        it('We should be able to command and respond to ACK requests', (done) => {
            config = new StompConfig('slappochip', 'nowhere', 12345, 'user', 'password', false, null, true);
            config.testMode = true;
            client.connect(config).subscribe(() => {
                client.subscribeToDestination(testDestination, subId);
            });
            ...
            client.socketSubscriptionObserver.subscribe((message) => {
                ...
                client.clientSocket.triggerEvent('message', [
                    {
                        data: StompParser.marshal(newMessage.command, newMessage.headers, newMessage.body)
                    }
                ]);
            });
        });
        it('We should be able to handle STOMP errors via an echo', (done) => {
            client.connect(config)
                .pipe(mergeMap(() => {
                return client.subscribeToDestination(testDestination, subId);
            })).subscribe(() => expect(true).toBeFalsy(), // should not hit
            (error) => {
            ...
            client.socketSubscriptionObserver.subscribe(() => {
                let binMessage = StompParser.marshal(StompClient.STOMP_ERROR, { 'error-header': 'yellowduck123', subscription: subId }, StompParser.stringToArrayBuffer('computer said no.'));
                client.clientSocket.send(binMessage);
            });
        });
        it('We should be able to handle STOMP errors via a server event', (done) => {
            client.connect(config)
                .pipe(mergeMap(() => {
                return client.subscribeToDestination(testDestination, subId);
            })).subscribe(() => expect(true).toBeFalsy(), // should not hit
    ...
    describe('Given we are able to operate normally', () => {
        it('The client should be able to handle error conditions and out of sequence events', (done) => {
            client.connect(config)
                .pipe(mergeMap(() => {
                return client.subscribeToDestination(testDestination, subId);
            })).subscribe();
        ...
        it('The client send heartbeats if configured', (done) => {
            config = new StompConfig('slappochip', 'nowhere', 12345, 'user', 'password', false, null, true, 0, 150);
            config.testMode = true;
        ...
        it('The client starts sending heartbeats using custom setInterval function', (done) => {
            config = new StompConfig('slappochip', 'nowhere', 12345, 'user', 'password', false, null, true, 0, 150);
            config.testMode = true;
            let heartbeatCounter = 0;
            config.startIntervalFunction = (handler, timeout, ...args) => {
                return setInterval(() => {
                    heartbeatCounter++;
                    handler(...args);
                }, timeout, ...args);
...
//# sourceMappingURL=stomp.client.spec.js.map