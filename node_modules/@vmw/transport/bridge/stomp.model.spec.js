...
describe('Stomp Model [stomp.config]', () => {
    ...
    describe('Stomp Configuration', () => {
        it('We should be able to create a configuration and validate the properties are set', () => {
            let config = new StompConfig('/endpoint', 'somehost', 12345, 'user', 'pass');
            log = new Logger();
            log.silent(true);
            ...
            config = new StompConfig('/endpoint', 'somehost', 12345, 'user', 'pass', true, null, true);
        ...
        it('We should be able to generate a config statically', () => {
            const config = new StompConfig('/endpoint', 'somehost', 12345, 'user', 'pass', true, null, true, 50, 50);
        ...
        it('We should be able to generate a config with a different heartbeat', () => {
            let gen = StompConfig.generate('/somewhere', 'thebesthost', 31337, true, 'darth', 'vader');
        ...
        it('We should be able to create mock and real sockets', () => {
            let config = new StompConfig('/endpoint', 'somehost', 1080, 'user', 'pass');
            let s = config.generateSocket();
            ...
            s = config.generateSocket();
            expect(s.protocols).toContain('dummy');
            expect(s.protocols).toContain('access-token.mock-token');
        });
    });
    describe('Stomp Session', () => {
        it('We should be able to create a session and validate it', () => {
            let config = new StompConfig('/endpoint', 'somehost', -1, 'user', 'pass');
            config.testMode = true;
...
//# sourceMappingURL=stomp.model.spec.js.map