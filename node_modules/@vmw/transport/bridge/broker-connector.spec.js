...
import { BrokerConnectorChannel, StompConfig } from './stomp.model';
...
describe('BrokerConnector [broker-connector.ts]', () => {
    ...
    let config;
    let configNoTopics;
    let configMultiBroker;
    let configCustomId;
    let configApplicationPrefix;
    let log;
    let subId;
    const topicA = '/topic/testA';
    afterEach(() => {
    ...
    describe('Global headers should be passed to the StompSession', () => {
        beforeEach(() => {
            ...
            spyOn(bc['currentSessionMap'], 'set').and.callFake((key, value) => {
        ...
        it('CONNECT, SUBSCRIBE, UNSUBSCRIBE and DISCONNECT messages should include global headers', (done) => {
            ...
            const session = bc['currentSessionMap'].get(connString);
    ...
    describe('Service configuration and basic connect/disconnect', () => {
        it('We should be able to set the configuration', (done) => {
        ...
        it('We should be able to determine that we\'re connected', (done) => {
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
        ...
        it('We should be able to determine that we\'re disconnected', (done) => {
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
    ...
    describe('[Public channel] Subscribing, messaging and un-subscribing', () => {
        it('We should be able to subscribe to a broker destination', (done) => {
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        BrokerConnector.fireSubscribeCommand(bus, command.session, topicA, subId, false, '/topic');
                        break;
        ...
        it('We should be able to subscribe to a broker destination and send/receive messages', (done) => {
            let outboundMessage = 'a lovely horse';
            let mId = GeneralUtil.genUUIDShort();
            let headers = { id: mId, subscription: subId };
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                    ...
                    case StompClient.STOMP_SUBSCRIBED:
                        ...
                        let message = StompParser.generateStompBusCommand(StompClient.STOMP_SEND, command.session, topicA, StompParser.generateStompReadyMessage(outboundMessage, headers));
        ...
        it('We should be able to unsubscribe to a broker destination', (done) => {
            let sessId;
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
                    ...
                    case StompClient.STOMP_SUBSCRIBED:
                        expect(command.destination).toEqual(topicA);
                        BrokerConnector.fireUnSubscribeCommand(bus, command.session, topicA, subId, false, '/topic');
                        break;
    ...
    describe('[Private channel] Subscribing, messaging and un-subscribing', () => {
        it('We should be able to subscribe to a broker destination', (done) => {
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        BrokerConnector.fireSubscribeCommand(bus, command.session, topicA, subId, true, '/queue');
                        break;
                    case StompClient.STOMP_SUBSCRIBED:
                        expect(command.destination).toEqual(topicA);
                        done();
        ...
        it('We should be able to unsubscribe to a broker destination', (done) => {
            ...
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    ...
                    case StompClient.STOMP_SUBSCRIBED:
                        expect(command.destination).toEqual(topicA);
                        BrokerConnector.fireUnSubscribeCommand(bus, command.session, topicA, subId, true, '/queue');
                        break;
                    case StompClient.STOMP_UNSUBSCRIBED:
                        expect(command.destination).toEqual(topicA);
                        expect(command.session).toEqual(sessId);
    ...
    describe('Service should only respond to properly formed/valid messages', () => {
        it('We should only respond to valid connection messages sent on the bus', (done) => {
            ...
            let wrongCommand = StompParser.generateStompBusCommand(StompClient.STOMP_CONNECTED, '', '', config);
            bus.sendRequestMessage(BrokerConnectorChannel.connection, wrongCommand);
            expect(bc.connectClient).not.toHaveBeenCalled();
            done();
        });
        it('We should only respond to valid disconnection messages sent on the bus', (done) => {
            ...
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        ...
                        expect(bc.disconnectClient).not.toHaveBeenCalled();
                        // wrong command message
                        let wrongCommand = StompParser.generateStompBusCommand(StompClient.STOMP_DISCONNECTING, '', '', config);
                        bus.sendRequestMessage(BrokerConnectorChannel.connection, wrongCommand);
                        expect(bc.disconnectClient).not.toHaveBeenCalled();
                        done();
                        break;
        ...
        it('We should only respond to valid subscription messages sent on the bus', (done) => {
            ...
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        let missingProperties = StompParser.generateStompBusCommand(StompClient.STOMP_SUBSCRIBE, null, null, StompParser.generateStompBrokerSubscriptionRequest('123', null, null, false, '/topic'));
                        // public channel
                        ...
                        // private channel
                        missingProperties.payload = StompParser.generateStompBrokerSubscriptionRequest('123', null, null, true, '/queue');
                        expect(bc.subscribeToDestination).not.toHaveBeenCalled();
                        // create a bad bus message
                        bus.sendRequestMessage(BrokerConnectorChannel.connection, StompClient.STOMP_SUBSCRIBE);
                        expect(bc.subscribeToDestination).not.toHaveBeenCalled();
                        let wrongCommand = StompParser.generateStompBusCommand(StompClient.STOMP_SUBSCRIBED, '123', '234', StompParser.generateStompBrokerSubscriptionRequest('123', 'abc', '456', false, '/topic'));
                        ...
                        wrongCommand.payload = StompParser.generateStompBrokerSubscriptionRequest('123', 'abc', '456', true, '/queue');
                        bus.sendRequestMessage(BrokerConnectorChannel.subscription, wrongCommand);
                        expect(bc.subscribeToDestination).not.toHaveBeenCalled();
                        done();
                        break;
        ...
        it('[Public channel] We should only respond to valid unsubscription messages sent on the bus', (done) => {
            ...
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    ...
                    case StompClient.STOMP_SUBSCRIBED:
                        ...
                        let wrongCommand = StompParser.generateStompBusCommand(StompClient.STOMP_UNSUBSCRIBED, '123', '234', StompParser.generateStompBrokerSubscriptionRequest('123', 'abc', '456', false, '/topic'));
                        // send wrong command
                        bus.sendRequestMessage(BrokerConnectorChannel.subscription, wrongCommand);
                        expect(bc.unsubscribeFromDestination).not.toHaveBeenCalled();
                        let missingPayload = StompParser.generateStompBusCommand(StompClient.STOMP_UNSUBSCRIBE, null, null, null);
        ...
        it('[Private channel] We should only respond to valid unsubscription messages sent on the bus', (done) => {
            spyOn(bc, 'unsubscribeFromDestination')
                .and
                .callThrough();
            let sessId;
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    ...
                    case StompClient.STOMP_SUBSCRIBED:
                        ...
                        bus.sendRequestMessage(BrokerConnectorChannel.connection, StompClient.STOMP_UNSUBSCRIBE);
                        expect(bc.unsubscribeFromDestination).not.toHaveBeenCalled();
                        let wrongCommand = StompParser.generateStompBusCommand(StompClient.STOMP_UNSUBSCRIBED, '123', '234', StompParser.generateStompBrokerSubscriptionRequest('123', 'abc', '456', true, '/queue'));
                        // send wrong command
                        bus.sendRequestMessage(BrokerConnectorChannel.subscription, wrongCommand);
                        expect(bc.unsubscribeFromDestination).not.toHaveBeenCalled();
                        let missingPayload = StompParser.generateStompBusCommand(StompClient.STOMP_UNSUBSCRIBE, null, null, null);
        ...
        it('[Public channel] We should only respond to valid outbound messages sent on the bus', (done) => {
            ...
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        ...
                        break;
                    case StompClient.STOMP_SUBSCRIBED:
                        let message = StompParser.generateStompBusCommand(StompClient.STOMP_SEND, '', '', null);
                        // missing message payload
                        bus.sendRequestMessage(BrokerConnectorChannel.messages, message);
                        message = StompParser.generateStompBusCommand(StompClient.STOMP_MESSAGE, command.session, topicA, StompParser.generateStompReadyMessage(outboundMessage, headers));
                        // valid message, wrong command
                        bus.sendRequestMessage(BrokerConnectorChannel.messages, message);
                        message = StompParser.generateStompBusCommand(StompClient.STOMP_SEND, command.session, null, StompParser.generateStompReadyMessage(outboundMessage, headers));
        ...
        it('[Private channel] We should only respond to valid outbound messages sent on the bus', (done) => {
            let outboundMessage = 'anyone fancy a pint?';
            let mId = GeneralUtil.genUUIDShort();
            let headers = { id: mId, subscription: subId };
            spyOn(bc, 'sendPacket')
    ...
    describe('Error handling and socket failure recovery', () => {
        it('We should be able to handle errors on the socket correctly', (done) => {
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        let session = bc.getSession(command.session);
                        setTimeout(() => session.client.clientSocket.triggerEvent('error', ['unknown']));
                        break;
                    default:
                        break;
                }
        ...
        it('trying to subscribe with an invalid broker session should throw an error', (done) => {
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        BrokerConnector.fireSubscribeCommand(bus, 'bee-doh-bee-doh-bee-doh', topicA, subId, false, '/topic');
                        break;
                    default:
                        break;
                }
    ...
    describe('Galactic channel events and message bus monitor consumption', () => {
        it('We should be able to ensure that the stomp service is online on the monitor channel', (done) => {
            /*
        ...
        it('The monitor stream message handler should be able to drop any invalid monitor events', (done) => {
            /*
            ...
            const mo = new MonitorObject().build(MonitorType.MonitorNewChannel, null, null);
            bus.sendRequestMessage(MonitorChannel.stream, mo);
        });
        it('galactic channels created before the stomp client has connected should be proxied once connected', (done) => {
            /*
            ...
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    case StompClient.STOMP_SUBSCRIBED:
                        ...
                        // trigger an unsubscription via channel close
                        bus.closeGalacticChannel('happy-puppers', getName(), { brokerIdentity: 'somehost:12345somewhere', isPrivate: false });
                        break;
                    case StompClient.STOMP_UNSUBSCRIBED:
                        bc.sessions.forEach((session) => {
            ...
            bus.api.getGalacticChannel('happy-puppers', { brokerIdentity: 'somehost:12345somewhere', isPrivate: false }, getName(), false);
            bus.api.getGalacticChannel('naughty-kitties', { brokerIdentity: 'somehost:12345somewhere', isPrivate: false }, getName(), false);
            BrokerConnector.fireConnectCommand(bus, config);
        });
        it('check galactic channels can operate against low level messages', (done) => {
            let count = 0;
            /*
             ...
             */
            const chan = bus.api.getGalacticChannel('bouncy-pups', { brokerIdentity: 'somehost:12345somewhere', isPrivate: false }, getName(), false);
            chan.subscribe((msg) => {
        ...
        it('check galactic messages are sent when sessions are in play', (done) => {
            ...
            bus.api.getGalacticChannel('sometopic', { brokerIdentity: 'somehost:12345somewhere', isPrivate: false }, getName(), false);
            BrokerConnector.fireConnectCommand(bus, config);
        });
        it('[Public channel] check galactic messages are sent with or without application prefix ', (done) => {
            ...
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        bus.sendGalacticMessage('sometopic', 'hello!');
                        bus.api.tickEventLoop(() => {
                            expect(log.debug).toHaveBeenCalledWith('Sending Galactic Message for session anything to ' +
        ...
        it('[Private channel] check galactic messages are sent with or without application prefix ', (done) => {
            const debugLogMessages = [];
            spyOn(log, 'debug').and.callFake((msg) => {
            ...
            bus.api.getGalacticChannel('sometopic', { brokerIdentity: 'somehost:12345somewhere', isPrivate: true }, getName(), false);
            BrokerConnector.fireConnectCommand(bus, configApplicationPrefix);
        });
        it('check galactic channels are not closed if there are no sessions active.', (done) => {
            spyOn(log, 'warn').and.callThrough();
            spyOn(log, 'info').and.callThrough();
            /**
             * Check that galactic channels cannot be closed if topics are no configured
             */
            const brokerIdentity = { brokerIdentity: 'somehost:12345somewhere', isPrivate: false };
            ...
            bus.api.tickEventLoop(() => {
                ...
                expect(log.warn)
                    .toHaveBeenCalledWith('unable to close galactic channel, no open sessions.', bc.getName());
                done();
            });
        });
        it('check that incoming stomp subscriptions are not processed if they are invalid commands', (done) => {
            spyOn(log, 'warn').and.callThrough();
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        ...
                        BrokerConnector.fireSubscriptionCommand(bus, '123', 'somewhere', '123456', StompClient.STOMP_ABORT, false, '/topic');
                        bus.api.tickEventLoop(() => {
                            expect(log.warn)
                                .toHaveBeenCalledWith('unable to validate inbound subscription message, invalid', bc.getName());
                            done();
                        });
                        break;
                    default:
        ...
        it('check that a broker with multiple relays responds correctly.', (done) => {
            spyOn(log, 'info').and.callThrough();
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
        ...
        it('sendPacket() works with and without the correct headers and session properties.', (done) => {
            spyOn(log, 'warn').and.callThrough();
            spyOn(log, 'debug').and.callThrough();
            bc.sendPacket({
                payload: {
        ...
        it('subscribeToDestination() forwards any message sent on galactic channel', (done) => {
            spyOn(bc, 'sendPacket').and.callThrough();
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
        ...
        it('[Public channel] unsubscribeFromDestination() behaves correctly if incorrect session state exists.', (done) => {
            spyOn(log, 'warn').and.callThrough();
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
        ...
        it('[Private channel] unsubscribeFromDestination() behaves correctly if incorrect session state exists.', (done) => {
            spyOn(log, 'warn').and.callThrough();
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
        ...
        it('[Public channel] unsubscribeFromDestination() behaves correctly if no galactic subscriptions can be found.', (done) => {
            spyOn(log, 'warn').and.callThrough();
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
        ...
        it('[Private channel] unsubscribeFromDestination() behaves correctly if no galactic subscriptions can be found.', (done) => {
            spyOn(log, 'warn').and.callThrough();
            bus.listenStream(BrokerConnectorChannel.status)
                .handle((command) => {
                switch (command.command) {
        ...
        it('Make sure the reconnect timer triggers when socket is closed.', (done) => {
            ...
            const fireClose = () => {
                bc.sessions.forEach((session) => {
                    const socket = session.client.clientSocket;
            ...
            bus.listenStream(BrokerConnectorChannel.status)
                ...
                switch (command.command) {
                    case StompClient.STOMP_CONNECTED:
                        ...
                        bus.api.tickEventLoop(() => {
                            expect(log.warn)
                                .toHaveBeenCalledWith('WebSocket to broker closed!', bc.getName());
                            expect(log.info)
                                .toHaveBeenCalledWith('Starting re-connection timer', bc.getName());
                            expect(log.warn)
                                .toHaveBeenCalledWith('Trying to reconnect to broker....', bc.getName());
                            done();
                        }, 80);
                        break;
                    default:
...
function setTestMode(config) {
    config.testMode = true;
}
// helper to create a standard mocked config
function createStandardConfig(useTopics = true, multiBroker = false, customSession, applicationPrefix, host = 'somehost', port = 12345, endpoint = 'somewhere') {
    let configuration = new StompConfig(endpoint, host, port, '', '', false, applicationPrefix);
...
//# sourceMappingURL=broker-connector.spec.js.map