...
describe('Stomp Validator [stomp.validator]', () => {
    ...
    it('Check that monitor messages can be validated', () => {
        let mo = new MonitorObject().build(MonitorType.MonitorNewChannel, '#bleep-blip-bloop', 'somewhereOuthere');
...
function generateBusCommand(cmd, id, msg, destination = 'somewhere') {
    return StompParser.generateStompBusCommand(cmd, id, destination, msg);
}
function packageStompMessage(cmd, id, msg) {
    return new Message().request(generateBusCommand(cmd, id, msg));
}
//# sourceMappingURL=stomp.validator.spec.js.map