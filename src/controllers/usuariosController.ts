...
class UsuariosController {
  public async list(req: Request, res: Response) {
    await pool.query(
      "SELECT u.id_usuario, u.nombre, u.apellido,c.nombre_carrera,c.id_carrera,u.unidad_educativa,u.correo_electronico,u.contrasenia, r.tipo_rol ,u.id_rol from usuario u, rol r, carreras_fica c where u.id_rol=r.id_rol and c.id_carrera=u.id_carrera ORDER BY fecha_registro DESC ",
      (err: any, rows: any) => {
        if (err) {
  ...
  public async getOne(req: Request, res: Response) {
    ...
    const usuarios = await pool.query(
      " SELECT  u.id_usuario, u.nombre, u.apellido,c.nombre_carrera,c.id_carrera,u.unidad_educativa,u.correo_electronico,u.contrasenia, r.tipo_rol ,u.id_rol from usuario u, rol r, carreras_fica c WHERE  r.id_rol=u.id_rol and c.id_carrera=u.id_carrera and u.id_usuario=?",
      [id]
    );
  ...
  public async create(req: Request, res: Response) {
    ...
    if (findCorreo.length > 0) {
      ...
      if (id_rol == 4) {
        ...
        try {
          const query =
            "INSERT INTO usuario ( nombre,apellido,unidad_educativa,correo_electronico,contrasenia, id_carrera,id_rol) VALUES (?,?,?,?,?,(select id_carrera from carreras_fica where nombre_carrera=?),?)";
  ...
  public async update(req: Request, res: Response) {
    ...
    try {
      ...
      const query =
        "UPDATE usuario set nombre=?,apellido=?,id_carrera=(select id_carrera from carreras_fica where nombre_carrera=?),unidad_educativa=?,correo_electronico=?,contrasenia=?, id_rol=(select id_rol from rol where tipo_rol=?) WHERE id_usuario=?";
      pool.query(query, [
        nombre,
        apellido,
