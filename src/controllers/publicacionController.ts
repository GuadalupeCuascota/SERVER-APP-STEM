...
class ArchivosController {
  ...
  public async getOne(req: Request, res: Response) {
    ...
    const publicacion = await pool.query(
      "SELECT p.id_publicacion, p.titulo, p.nombre_perfil, p.fecha_publicacion, p.descripcion, p.estado_profesion, p.ruta_archivo, p.public_id_archivo, p.tipo_archivo, p.id_tipo_publicacion, p.id_usuario, p.id_estado_publicacion, p.id_carrera,c.nombre_carrera,p.enlace ,p.profesion FROM publicacion p , carreras_fica c   WHERE  p.id_carrera=c.id_carrera and p.id_publicacion=?",
      [id]
    );
  ...
  public async update(req: Request, res: Response) {
    ...
    try {
      ...
      if (req.file) {
        ...
        try {
          const query =
          "UPDATE publicacion set titulo=?,nombre_perfil=?,descripcion=?,enlace=?, profesion=?,estado_profesion=?, ruta_archivo=? ,tipo_archivo=?,public_id_archivo=?, id_carrera=(select id_carrera from carreras_fica where nombre_carrera=?) WHERE id_publicacion=?";
        ...
        try {
          const query =
            "UPDATE publicacion set titulo=?,nombre_perfil=?,descripcion=?,enlace=?, profesion=?,estado_profesion=?, ruta_archivo=?,  id_carrera=(select id_carrera from carreras_fica where nombre_carrera=?) WHERE id_publicacion=?";
